# -*- coding: utf-8 -*-
"""DEMO IV Charters: Value & Org Extraction.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qlNAER51luDp4yLGZUazs9rneZet59oR
...
def _extract_constraint_values_from_region(subdoc, attention_vector_name='values'):
  ...
  vector = subdoc.distances_per_pattern_dict[attention_vector_name]
  _sentences_having_currency_numbers, _, _ = _find_sentences_by_attention_vector(subdoc, vector)

 
  for start in _sentences_having_currency_numbers:
    end = _sentences_having_currency_numbers[start]

    section = subdoc.subdoc(start, end)
    results, m = extract_sums_from_tokens(section.tokens, section.distances_per_pattern_dict[attention_vector_name])

    sentence ={
        'quote':untokenize(section.tokens_cc),
        'subdoc':section,
        'constraints':[ r['sum'] for r in  results]
    }
...
def _clean_row(worksheet, r, s, e):
  for col in range(s, e + 1):
    worksheet.update_cell(r, col, '')
