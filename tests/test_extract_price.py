...
data = [
  (-1, 2388120.0, 'RUB',
   'Предельная стоимость услуг, перечисленных в Приложении №1 договора, за период указанный в пункт 7.1.1. '
   'договора, составляет 2388120 (Два миллиона триста восемьдесят восемь тысяч сто двадцать) рублей 00 копеек, '
   'кроме того НДС (20%) в размере 477624 (Четырёхсот семидесяти семи тысяч шестисот двадцати четырёх) рублей 00 копеек, '
   'всего, с учетом НДС 20%, 2865744 (Два миллиона восемьсот шестьдесят пять тысяч семьсот сорок четыре) рубля 00 копеек.'),

  (0, 188156900 - 28701900, 'RUB',
   '4.1. Стоимость выполняемых Подрядчиком Работ по '
   'Договору составляет 188156900 (сто восемьдесят восемь миллионов сто '
   'пятьдесят шесть тысяч девятьсот рублей 00 копеек), в том числе НДС 28701900 (двадцать восемь миллионов семьсот одна тысяча '
   'девятьсот рублей 00 копеек), и определена на основании Смет (Приложение №6 к Договору) объединенных '
   'Сводной сметой (Приложение №5 к Договору).'),

  (0, 2800567.46 - 427205.21, 'RUB',
   'Стоимость выполняемых Подрядчиком Работ по Договору составляет   2800567,46 (  Два миллиона восемьсот тысяч пятьсот '
   'шестьдесят семь рублей   46 копеек), в том числе НДС   427205,21 (  Четыреста двадцать семь тысяч двести пять рублей   21 коп.), и '
   'определена на основании Смет (Приложение №6 к Договору) объединенных Сводной сметой (Приложение №5 к Договору).'),

  (-1, 7621761.60 - 1162641.60, 'RUB',
   'Стоимость выполняемых Исполнителем по Договору дополнительных работ по Авторскому надзору '
   '(в том числе Технического сопровождения) за строительством объекта составляет не более 7621761,60 (семь миллионов шестьсот двадцать '
   'одна тысяча семьсот шестьдесят один рубль 60 копеек), в том числе НДС 1162641,60 (один миллион сто шестьдесят две тысячи шестьсот сорок один рубль 60 копеек)'),
...
class PriceExtractTestCase(unittest.TestCase):
  ...
  def test_find_all_value_sign_currency(self):
    ...
    text = """стоимость, равную или превышающую 2000000 ( два миллиона ) долларов США, но менее"""
    doc: ContractDocument = ContractDocument(text)
    doc.parse()
  ...
  def test_extract(self):
    ...
    for (sign, price, currency, text) in data:
      ...
      try:
        ...
        msg = f"\033[1;35;40m FAILED: expected: {price}  {currency}  {normal_text}  actual={f}"
        print(msg)
        failures.append(msg)
        print(sys.exc_info())
