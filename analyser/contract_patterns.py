...
class ContractPatternFactory(AbstractPatternFactoryLowCase):
  ...
  def _build_subject_patterns(self):
    ...
    if True:
      ...
      cp('Арендодатель обязуется предоставить',
         'Арендатору',
      ...
      cp('Заказчик поручает и оплачивает, а Исполнитель предоставляет ', 'услуги', 'в виде')
      cp('договор на оказание', 'платных', 'услуг')
      cp('договор', 'возмездного', 'оказания услуг')
    ...
    if True:
      ...
      cp('', 'имущество, земельные участки, жилое, нежилое, здание', 'площадь, находящееся по адресу')
    ...
    if True:
      ...
      cp('Заемщик обязуется вернуть указанную', 'сумму займа', 'твместе с причитающимися процентами')
